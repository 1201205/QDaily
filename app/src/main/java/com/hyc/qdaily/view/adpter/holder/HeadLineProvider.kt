 ...
 */
class HeadLineProvider : ItemViewProvider<HeadLineProvider.HeadLineHolder, ViewData<HeadLine>>() {
    override fun onBindViewHolder(holder: HeadLineHolder, data: ViewData<HeadLine>, position: Int, wrapper: ParamWrapper) {
        var headLine = data.content
        headLine?.list?.let {
            with(holder) {
                if (data.content!!.list?.size == 0) {
                    return
                }
                tvLine1.text = headLine!!.list!![0].description
                tvLine2.text = headLine!!.list!![1].description
                tvLine3.text = headLine!!.list!![2].description
            }
        }
